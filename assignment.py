        ...
        while i < len(parameters):
            if parameters[i] == "TopRight":
                if i + 2 >= len(parameters):
                    return False
                self.top = self._parse_int(parameters[i + 1])
                self.right = self._parse_int(parameters[i + 2])
                i += 3
    ...
    def parse(self, parameters: List[str]) -> bool:
        ...
        while i < len(parameters):
            if parameters[i] == "TopRight":
                ...
                if i + 2 >= len(parameters):
                    return False
                self.bottom = self._parse_int(parameters[i + 1])
                self.left = self._parse_int(parameters[i + 2])
                i += 3
            else:
                return False
        if (self.top is None
